...
CzLRMWallet::CzLRMWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZLRMSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZLRMSeed_deprecated()) {
                    LogPrintf("%s: Updated zLRM seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzLRMWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZLRMCount(nCountLastUsed);
    else if (!walletdb.ReadZLRMCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzLRMWallet::Lock()
{
    seedMaster = 0;
}

void CzLRMWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzLRMWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZLRM(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzLRMWallet::LoadMintPoolFromDB()
{
...
void CzLRMWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzLRMWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzLRMWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzLRMWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZLRM(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzLRMWallet::SeedToZLRM(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzLRMWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzLRMWallet::GenerateDeterministicZLRM(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzLRMWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzLRMWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
