...
// Copyright (c) 2016-2018 The LRMcoin developers
...
public:
    ...
    bool WriteZLRMSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZLRMSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZLRMSeed_deprecated(uint256& seed);
    bool EraseZLRMSeed();
    bool EraseZLRMSeed_deprecated();

    bool WriteZLRMCount(const uint32_t& nCount);
    bool ReadZLRMCount(uint32_t& nCount);
