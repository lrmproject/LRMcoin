...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 LRM at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zLRM send
enum ZerocoinSpendStatus {
    ZLRM_SPEND_OKAY = 0,                            // No error
    ZLRM_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZLRM_WALLET_LOCKED = 2,                         // Wallet was locked
    ZLRM_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZLRM_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZLRM_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZLRM_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZLRM_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZLRM_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZLRM_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZLRM_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZLRM_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZLRM_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZLRM_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZLRM_SPENT_USED_ZLRM = 14,                      // Coin has already been spend
    ZLRM_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZLRM_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZLRMOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzLRMWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzLRMTracker> zpivTracker;
    ...
    void setZWallet(CzLRMWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpivTracker = std::unique_ptr<CzLRMTracker>(new CzLRMTracker(strWalletFile));
    }

    CzLRMWallet* getZWallet() { return zwalletMain; }
    ...
    /** zLRM reset */
    boost::signals2::signal<void()> NotifyzLRMReset;
