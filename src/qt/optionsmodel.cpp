...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::LRM);
    ...
    if (!settings.contains("nAnonymizeLrmcoinAmount"))
        settings.setValue("nAnonymizeLrmcoinAmount", 1000);

    nAnonymizeLrmcoinAmount = settings.value("nAnonymizeLrmcoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeLrmcoinAmount"))
        SoftSetArg("-anonymizelrmcoinamount", settings.value("nAnonymizeLrmcoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in lrmcoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeLrmcoinAmount:
            return QVariant(nAnonymizeLrmcoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeLrmcoinAmount:
            nAnonymizeLrmcoinAmount = value.toInt();
            settings.setValue("nAnonymizeLrmcoinAmount", nAnonymizeLrmcoinAmount);
            emit anonymizeLrmcoinAmountChanged(nAnonymizeLrmcoinAmount);
            break;
        case CoinControlFeatures:
