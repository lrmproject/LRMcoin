...
#ifndef LRMcoin_ZLRMWALLET_H
#define LRMcoin_ZLRMWALLET_H
...
public:
    CzLRMWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZLRM(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZLRM(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //LRMcoin_ZLRMWALLET_H
